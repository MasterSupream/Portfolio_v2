...
export function Waves({
    ...
    // Performance optimization: throttle animation to 60fps max
    const TARGET_FPS = 60
    ...
    // Optimized line setup with reduced density for better performance
    const setLines = useCallback(() => {
        ...
        // Optimize spacing based on screen size for better performance
        const isMobile = width < 768
        const xGap = isMobile ? 12 : 10  // Larger gaps on mobile for performance
        const yGap = isMobile ? 12 : 10

        const oWidth = width + 100  // Reduced padding
        const oHeight = height + 20

        const totalLines = Math.min(Math.ceil(oWidth / xGap), isMobile ? 50 : 80) // Limit lines
        const totalPoints = Math.min(Math.ceil(oHeight / yGap), isMobile ? 40 : 60) // Limit points

        const xStart = (width - xGap * totalLines) / 2
        const yStart = (height - yGap * totalPoints) / 2

        // Create vertical lines with performance optimization
        for (let i = 0; i < totalLines; i++) {
            ...
            // Create SVG path with optimizations
            ...
            path.setAttribute('vector-effect', 'non-scaling-stroke') // Performance optimization
    ...
    // Optimized animation logic with frame rate control
